...
<div class="container">
    <div class="menu">
        <ul>
            <li><h1><a href="<?=url($module.'/doc/index')?>">文档首页</a></h1></li>
            <?php foreach ($config as $i => $a) : ?>
                <li>
                    <h1><a href="<?=url($module.'/doc/'.$a['path'])?>"><?=$a['name']?></a></h1>
                    <?php if (!empty($a['children'])) : ?>
                        <ul>
                            <?php foreach ($a['children'] as $ii => $b) : ?>
                                <li>
                                    <h2><a href="<?=url($module.'/doc/'.$a['path'].'-'.$b['path'])?>"><?=$b['name']?></a></h2>
                                    <?php $files = getFile('', $doc); if (!empty($files)) : ?>
                                        <ul>
                                            <?php $files = getFile($a['path'].'/'.$b['path'], $doc);  foreach ($files as $file) : ?>
                                                <li>
                                                    <h3><a id="anchor-<?=md5($i.$ii.$file)?>" href="<?=url($module.'/doc/'.$a['path'].'-'.$b['path'].'-'.$file).'#anchor-'.md5($i.$ii.$file)?>"><?=$file?></a></h3>
                                                </li>
                                            <?php endforeach; ?>
                                        </ul>
                                    <?php endif; ?>
